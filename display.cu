...
// where before there would be crashes on linux
...
int CudaDisplay::attachShader( GLenum type, const char* path)
{
	...
	memset(src, 0, size + 1);
	fseek(f, 0, SEEK_SET);	
	int rc = fread(src, 1, size, f);
	if (!rc) throw "Unable to read shader";
...
int CudaDisplay::render(const Image* img)
{
	int rc;
	if (img)
	{
		rc = cudaMemcpy2D(
		GL.pbaddr, CUDA.frame.width * sizeof(float4),
		img->mem.device.data, img->mem.device.pitch,
	...
	else
	{
		...
		CUDA.frame.width * sizeof(float4),
		CUDA.frame.height,
		cudaMemcpyDeviceToHost);
	}
	...
	if (cudaSuccess != cudaGetLastError()) throw "General CUDA error";
...
CudaDisplay::~CudaDisplay()
{
	...
	if (GL.pbres)
	{
		cudaGraphicsUnmapResources(1, &GL.pbres, 0);
	}
